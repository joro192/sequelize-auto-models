  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        ...
        text[table] = "const sequelize = require(\"../config/env.js\");\n";
        text[table] += "const Sequelize = require(\"sequelize\");\n";
        text[table] += "const Model = Sequelize.Model;\n\n";
        // text[table] += "module.exports = function(sequelize, Sequelize) {\n";
        // text[table] += spaces + "return sequelize.define('" + tableName + "', {\n";
        text[table] += "class " + tableName +" extends Model {}\n";
        text[table] += tableName + ".init({\n";
      } else {
      ...
      text[table] += spaces + spaces  + "modelName: '" + table + "',\n";
      ...
      text[table] += "\n);\n\n";
      text[table] += "module.exports = "+ tableName +";\n";
      _callback(null);
    }, function(){
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    ...
    tables.forEach((model, index) => {
      let name = model.toLowerCase();
      if (name != "index")
      modelGen += `   db.${name} = require("./${model}.js");\n`;
    });
    fs.writeFileSync(path.join(self.options.directory, 'index.js'), `module.exports = genModel = db => {\n${modelGen}}`, 'utf8');
  }
